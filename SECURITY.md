...
it by **position** (`Key 1/2 exhausted`), never by value. `atlas-kit doctor` reports how
many keys are configured, never a key itself — there is a test that asserts a key value
cannot reach stdout.

`.env` is never read by atlas-kit. Only `.env.example` is versioned.
...
atlas-kit does not scan for secrets before indexing: a secret in a variable's *value*
...
(`pip install 'atlas-kit[local]'`) does the same work on-device with no network call.
