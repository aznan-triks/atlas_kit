...
Shape stored under `codex["edges"]`:

    {"language": "python",
     "imports": {"<rel/posix.py>": ["os", "code_fauna_codex.scan", ...]},
...
def _callee_name(func: ast.expr) -> str | None:
    ...
    Only the last segment is stored, deliberately. Codex symbol names are qualnames
...
def parse_python_edges(path: Path, rel: str) -> tuple[list[str], list[dict]]:
    ...
    of aborting the scan — one broken file must never cost the whole codex.
    """
    try:
    ...
    # several branches is one edge, and the codex stays small and byte-stable across rescans.
    ...
    class Visitor(ast.NodeVisitor):
        ...
        def _visit_func(self, node) -> None:
            ...
            # enclosing-function prefix), so a `caller` is always literally a codex symbol name.
...
def assemble_edges(imports_by_file: dict[str, list[str]],
    ...
    """Merge per-file results into the `codex["edges"]` block, deterministically sorted.
...
def previous_edges_by_file(previous: dict | None) -> tuple[dict[str, list[str]], dict[str, list[dict]]]:
    """Regroup a previous codex's edges per file, so an unchanged file can be reused
    verbatim instead of re-parsed. Mirrors how `build_codex` reuses symbols."""
...
def callers_of(codex: dict, name: str) -> list[dict]:
    """Edges whose callee matches `name` (compared against the last dotted segment of `name`)."""
    target = _last_segment(name)
    return [row for row in codex.get("edges", {}).get("calls", []) if row["callee"] == target]


def callees_of(codex: dict, name: str) -> list[dict]:
    """Edges whose caller matches `name` (exact qualname match, or last-segment match)."""
    target = _last_segment(name)
    return [row for row in codex.get("edges", {}).get("calls", [])
            if row["caller"] == name or _last_segment(row["caller"]) == target]


def unreferenced_symbols(codex: dict) -> list[dict]:
    """Python symbols whose name never appears as any callee anywhere in the codex.
    ...
    """
    referenced = {row["callee"] for row in codex.get("edges", {}).get("calls", [])}
