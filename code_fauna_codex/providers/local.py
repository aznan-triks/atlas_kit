...
Optional dependency: pip install 'code-fauna-codex[local]'. `_default_embed_fn` is the
...
def _default_embed_fn(model_name: str, texts: list[str]) -> list[list[float]]:
    try:
        ...
        raise EmbeddingError(
            "Local provider needs the 'fastembed' package: pip install 'code-fauna-codex[local]'."
        ) from exc
