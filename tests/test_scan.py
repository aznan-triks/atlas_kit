...
def test_build_atlas_scans_python_and_js(tmp_path):
    ...
    names = {s["name"] for section in atlas["symbols"].values() for s in section}
    assert "foo" in names
    assert "bar" in names
    assert "skipped" not in names
    assert set(atlas["files"]) == {"a.py", "b.js"}


def test_build_atlas_incremental_skips_unchanged_files(tmp_path):
    write(tmp_path, "a.py", "def foo():\n    pass\n")
    first = build_atlas(tmp_path)
    ...
    second = build_atlas(tmp_path, previous=first)
    assert second["files"]["a.py"] == first["files"]["a.py"]


def test_build_atlas_incremental_updates_changed_file(tmp_path):
    write(tmp_path, "a.py", "def foo():\n    pass\n")
    first = build_atlas(tmp_path)
    write(tmp_path, "a.py", "def foo():\n    pass\n\ndef bar():\n    pass\n")
    second = build_atlas(tmp_path, previous=first)
