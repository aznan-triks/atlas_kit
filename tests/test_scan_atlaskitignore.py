"""Tests — the `.atlaskitignore` file at the scan root (union with `--ignore`, never a replacement)."""
from __future__ import annotations

from conftest import write

from atlas_kit.scan import build_atlas, read_ignore_file
...
def test_read_ignore_file_skips_comments_and_blank_lines(tmp_path):
    write(tmp_path, ".atlaskitignore", "# a comment\n\n  generated/*  \n\n#another\nvendored/*\n")
    assert read_ignore_file(tmp_path) == ["generated/*", "vendored/*"]


def test_build_atlas_excludes_files_matched_by_atlaskitignore(tmp_path):
    write(tmp_path, "keep.py", "def kept():\n    pass\n")
    write(tmp_path, "generated/mod.py", "def generated_symbol():\n    pass\n")
    write(tmp_path, ".atlaskitignore", "generated/*\n")
...
def test_atlaskitignore_is_a_union_with_the_ignore_argument(tmp_path):
    write(tmp_path, "keep.py", "def kept():\n    pass\n")
...
def test_atlaskitignore_also_excludes_edges_for_the_ignored_file(tmp_path):
    write(tmp_path, "keep.py", "def kept():\n    import os\n    os.getcwd()\n")
